...
INTELLIGENT COMMAND ENGINE v22
------------------------------
Semantic command execution with strict ambiguity handling.

Fixes in v22:
  • "all Hydrangea" no longer matches every row — only bare "all"
    is a wildcard. "all X" is treated as filter "X".
  • Compound commands separated by ";" or " then " run in sequence.
  • Typo tolerance via Levenshtein distance (≤ 1 edit for words ≥ 5 chars).
  • Never silently no-ops: unresolved instructions produce a clarification.

Integrates RapidFuzz for entity matching [citation:1][citation:10] and
Levenshtein for typo tolerance [citation:15].
"""

from __future__ import annotations
import os, re, json, math, logging, urllib.request, urllib.error
...
ROLE_ALIASES = {
    "product_name": ["product", "product name", "item", "item name",
                     ...
                     "item description", "particulars", "goods",
                     "article", "articles", "flower variety", "variety",
                     "cultivar"],
...
STOPWORDS = {"a", "an", "the", "of", "to", "for", "all", "and", "or",
             ...
             "make", "create", "add", "new", "column", "field",
             "called", "named", "that", "which", "using", "from",
             "into", "set", "change", "update", "apply", "fill",
             "put", "use"}
...
def _lev(a: str, b: str) -> int:
    """Levenshtein distance — typo tolerance [citation:15]."""
    if a == b:
        return 0
    if not a:
        return len(b)
...
def resolve_column(columns, query, numeric=False):
    ...
    if numeric:
        candidates = [x for x in candidates
                      if x[1].get("role") in NUMERIC_ROLES]
    if not candidates:
        return None, []
...
def _ok(items, columns, explanation, via="deterministic", **extra):
    return {"status": "ok", "success": True, "items": items,
...
# ===========================================================================
#  TARGET RESOLUTION — THE CRITICAL FIX
# ===========================================================================
WILDCARDS = {"all", "every", "each", "everything", "all rows", "all items",
             "all flowers", "all products", "everything here"}
...
def _is_all(target: str) -> bool:
    ...
    if not stripped:
        return True
    generic = {"rows", "row", "items", "item", "entries", "lines",
               "line", "flowers", "flower", "products", "product",
               "records", "everything", "here"}
...
def _target_rows(items, columns, target):
    ...
    # Strip the "all/every/each" prefix — the noun after is the filter
    t = _norm(target)
...
def _add_column(columns, label, role=None):
    ...
    while key in keys:
        key = f"{base}_{i}"
        i += 1
    return list(columns) + [{"key": key, "label": label, "role": role,
                             "source": "added"}], key
...
def _parse_price_command(msg: str):
    ...
    # Verb-less: "To all X unit price V"
    m = re.match(
    ...
    # Verbed, target-first: "set X price to 1.3"
    m = re.match(
...
def _calculate_totals(items, columns):
    ...
    if not q or not p:
        return _clarify(items, columns,
                        "I need Quantity and Unit Price before I can "
...
def _compute(items, columns, msg):
    ...
    m = re.search(
        ...
        rf"(?:column\s+)?([a-z0-9 _\-]+?)\s+(?:by|as|=)\s+", low)
    label = (m.group(1).strip().title() if m
             else ("Line Total" if op == "multiply" else "Computed"))

    columns, key = _add_column(columns, label,
                               "total" if op == "multiply" else None)
    out = deepcopy(items)
    for row in out:
...
def _modify(items, columns, msg):
    ...
    m = re.match(r"(?:remove|delete|exclude|drop)\s+(?:all\s+)?"
                 r"(.+?)(?:\s+rows?)?$", low)
    if m:
...
def _ai_plan(message, items, columns):
    ...
    system = """Semantic planner for document commands. Return ONE JSON:
{"action":"set_value|calculate_totals|grand_total|compute|remove_rows|
 remove_column|sort|clarify|none",
 "target":"","field":"","value":null,"operation":"","new_column":"",
 "explanation":""}
Never invent values. If unclear, use action "clarify"."""

    payload = json.dumps({"message": message, "columns": columns,
                          "sample_rows": items[:15]}, ensure_ascii=False)
    ...
    if provider == "groq" or (provider == "auto" and not key):
        ...
        data = {"model": os.getenv("GROQ_MODEL",
        ...
        try:
            req = urllib.request.Request(
                endpoint, data=json.dumps(data).encode(),
...
def _execute_ai_plan(items, columns, plan):
    ...
    if action == "set_value":
        ...
        if value is None:
            return _clarify(items, columns, "What value should I apply?", [])
    ...
    if action == "remove_rows":
        return _modify(items, columns, "remove " + str(plan.get("target") or ""))
    if action == "remove_column":
        return _modify(items, columns,
                       "remove column " + str(plan.get("field") or ""))
    if action == "sort":
        return _sort(items, columns, "sort by " + str(plan.get("field") or ""))
    if action == "clarify":
        return _clarify(items, columns,
...
def process_message(items, columns, message):
    ...
    if pending:
        for p in pending:
            explanations.append(f"Not applied because ambiguous: {p}")

    return {"status": "ok", "success": True, "items": current_items,
            "columns": current_columns,
            "explanation": "\n".join(f"{i+1}. {x}"
                                     for i, x in enumerate(explanations)),
